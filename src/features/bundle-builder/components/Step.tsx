...
export function Step({ step }: { step: IStep }) {
  ...
  return (
    <AccordionItem
      className={`border-b-[0.5px]  border-black-1F-headerAndBorder ${isOpen ? 'lg:bg-purple-secondary md:rounded-[10px] md:border-b-0' : ''}`}
      value={id}
    >
      <div className="border-b-[0.5px] border-black-1F-headerAndBorder px-[15px] pb-2 pt-3 text-10px uppercase leading-none tracking-[1.6px] text-gray-48-sub-header">
        {order} of 4 steps
      </div>
      <AccordionTrigger className="items-center gap-4 rounded-none border-transparent px-[15px] py-5 text-18px font-semibold leading-none text-black-10-header hover:no-underline [&>svg]:hidden">
        <span className="flex min-w-0 items-center gap-2 whitespace-nowrap">
          <img src={icon} alt={title} className="size-6 shrink-0" />
          {title}
        </span>
